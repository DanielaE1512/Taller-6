...
public class Aplicacion {
	...
	private void ejecutarAgregarElementoPedido() throws Exception
	{
		ArrayList<Producto> productosmenu= restaurante.getMenuBase();
		ArrayList<Combo> combos = restaurante.getCombos();
		ArrayList<Ingrediente> ingredientes = restaurante.getIngredientes();
		...
		if (tipo == 1) 
		{
			...
			int numeroProducto = Integer.parseInt(input("Ingrese el número del producto deseado"));
			int cambios = Integer.parseInt(input("1) Modificar producto \n2) No modificar producto"));

			if (cambios == 2)
			{
				pedido.agregarProducto(productosmenu.get(numeroProducto-1));
			}else if (cambios== 1)
			{
				ProductoMenu ajustado = (ProductoMenu)productosmenu.get(numeroProducto);
				ProductoAjustado productoAjustado= new ProductoAjustado(ajustado);
